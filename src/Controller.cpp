...
namespace lipm_walking
{
      ...
      stabilizer_(controlRobot(), pendulum_, dt)
  {
    ...
    LOG_SUCCESS("Handover with LIPMWalking controller init done " << this)
  }
  ...
  void Controller::internalReset()
  {
    ...
    controlRobot().setBaseLinkVelocity(Eigen::Vector6d::Zero()); // my PC
    // controlRobot().velW(sva::MotionVecd(Eigen::Vector6d::Zero()));

    realRobot().posW(X_0_fb);

   realRobot().setBaseLinkVelocity(Eigen::Vector6d::Zero()); // my PC
    // realRobot().velW(sva::MotionVecd(Eigen::Vector6d::Zero()));
...
CONTROLLER_CONSTRUCTOR("handoverLIPMWalking", lipm_walking::Controller)
